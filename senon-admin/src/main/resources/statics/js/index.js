    ...
    var IndexPage = {
        ...
        initButtonsEvent: function () {
            ...
            $("#manual-run-btn").click(function () {
                ...
                $.post(BASE_PATH + "/job/manualrun", {jobId: $("#viewing-job-input").val()}, function (res) {
                    Noty.info("已进入任务队列");
            ...
            $("#resume-run-btn").click(function () {
                ...
                $.post(BASE_PATH + "/job/resumerun", {jobId: $("#viewing-job-input").val()}, function (res) {
            ...
            $("#open-close-btn").click(function () {
                ...
                $.post(BASE_PATH + "/job/trigger_job", { 'jobId' : $("#viewing-job-input").val()}, function (res) {
        ...
        viewLog: function (historyId) {
            ...
            $.post(BASE_PATH + "/job/gethistorylog", {historyId: historyId}, function (res) {
                $("#log-his-p").html("");
                $("#logModal").modal("show");
        ...
        killJob : function(historyId){
        	...
        	if(type=="hive"){
        		  $.post(BASE_PATH+"/jobs/killhive",{historyId:historyId},function(res){
        			  if(res==1){
        				  alert("取消成功");
        			  }else  if(res==2){
        				  alert("取消失败");
        		 ...
        		 $.post(BASE_PATH+"/jobs/killshell",{historyId:historyId},function(res){
        			 if(res){
        				  alert("取消成功");
        			  }else{
        				  alert("取消失败");
        ...
        updateJob: function () {
            ...
            $.post(BASE_PATH + "/job/update_job", param,
                function (res) {
                    if (!res.succeed) {
